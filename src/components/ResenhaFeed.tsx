...
// Rótulos de cada categoria de prêmio da resenha (votados anonimamente ou detectados automaticamente)
...
export function ResenhaFeed() {
  ...
  const load = async () => {
    ...
    // "Grupo" mostra os posts dos grupos que você participa (RLS já garante isso sozinho).
    // "Todos" ainda não existe de verdade (depende do sistema de seguir jogadores, fase futura).
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {Object.entries(counts).map(([t, n]) => (
          <span key={t} className="px-2 py-0.5 rounded-full bg-secondary/60">
            {REACOES.find((r) => r.tipo === t)?.icon} {n as number}
          </span>
        ))}
      </div>
      ...
      return (
        <div>
          <div className="text-lg font-bold">
            <span className={premio.cor}>{premio.emoji} {c.nome} {premio.titulo}</span>
          </div>
