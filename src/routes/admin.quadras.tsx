...
export const Route = createFileRoute("/admin/quadras")({ component: Page });

function Page() {
  const [rows, setRows] = useState<any[]>([]);
  const [q, setQ] = useState("");
  const load = async () => {
    let qb = supabase.from("arenas").select("*").order("criado_em", { ascending: false } as any);
    if (q) qb = qb.or(`nome.ilike.%${q}%,cidade.ilike.%${q}%`);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        <Input placeholder="Buscar nome/cidade" value={q} onChange={e=>setQ(e.target.value)}/>
        ...
        {rows.map(r => (
          <Card key={r.id} className="p-3 flex justify-between items-center gap-2">
            <div>
              ...
              <div className="text-xs text-muted-foreground">{r.cidade}/{r.estado} · {r.plano ?? "gratuito"} · até {r.plano_validade?.slice(0,10) ?? "—"}</div>
            </div>
