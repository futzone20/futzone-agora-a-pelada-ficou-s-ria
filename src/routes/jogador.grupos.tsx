...
export const Route = createFileRoute("/jogador/grupos")({
  component: Grupos,
});
...
function Grupos() {
  ...
  const load = async () => {
    if (!user) return;
    // RLS já restringe isso aos grupos onde o usuário é membro (ou criador) — não precisa
    // filtrar aqui, a policy do Supabase cuida disso sozinha.
  ...
  return (
    <div className="space-y-4">
      ...
      {grupos.length === 0 ? (
        <EmptyState icon={Shield} title="Nenhum grupo ainda" description="Crie um grupo ou entre em um via convite pra começar." />
      ) : (
